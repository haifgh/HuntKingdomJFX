...
public class homeController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        lf = sf.follower(UserSession.getInstance().getUser().getId());
        follower.setText(""+lf.size());
        List<Follow> lfd= new ArrayList<>();
        lfd=sf.following(UserSession.getInstance().getUser().getId());
    ...
    private void setbar() {
         ...
         UserServices su = new UserServices();
         User u = new User();
         u=su.findById(UserSession.getInstance().getUser().getId());
  ...
  /*  public void UpdateA (int id) throws IOException  {
    ...
    VB.getChildren().clear();
     ...
     private void UpdatePost(int id )   {  
        ...
        le = sp.afficherpost(UserSession.getInstance().getUser().getId());
        for (Post evv : le) {
            u=su.findById(evv.getUser_id());
          lj = st.reloadjaime(UserSession.getInstance().getUser().getId(),evv.getId());   
         VBox h1 = new VBox();
         HBox h2 = new HBox();
         HBox h3= new HBox();  
          VB.setSpacing(5);
    ...
    private void HomeFollow()
        {  
         ...
         List<Jaime> lj = new ArrayList<>();
        //    lj = st.afficherjaime(UserSession.getUser().getId())
        le = sf.afficherpostfollow(UserSession.getInstance().getUser().getId());
        for (Post evv : le) {
         u = su.findById(evv.getUser_id());
         lj = st.reloadjaime(UserSession.getInstance().getUser().getId(),evv.getId());   
         VBox h1 = new VBox();
    ...
    private void searchUser() {
       ...
       VBB.getChildren().clear();
        ...
        for (Post evv : le) {
         ...
         final int f =u.getId();
         lf = sf.reloadfollow(UserSession.getInstance().getUser().getId(),u.getId());
           if (lf.size()!=0){
       ...
       public void singleDialogInformation(int recl) {
        ...
        UserServices su = new UserServices();
         User u = new User();
        lr = sr.afficherReclamation(UserSession.getInstance().getUser().getId());
